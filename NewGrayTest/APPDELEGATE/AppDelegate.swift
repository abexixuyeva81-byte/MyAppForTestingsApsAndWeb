...
// НОВЫЙ КОД: Возвращаем структуру для парсинга JSON из Flagsmith
struct Property: Codable {
...
class AppDelegate: UIResponder, UIApplicationDelegate, DeepLinkDelegate, UNUserNotificationCenterDelegate {
    ...
    // ... (остальные методы application(...), applicationDidBecomeActive(...) остаются без изменений) ...
    func application(_ application: UIApplication, supportedInterfaceOrientationsFor window: UIWindow?) -> UIInterfaceOrientationMask {
        return AppDelegate.orientationLock
    }
...
extension AppDelegate {
    ...
    // ИЗМЕНЕНО: Добавлена логика парсинга JSON
    func start(viewController: ViewController) {
        Logger.shared.log("Starting app flow, fetching remote config...")
        Flagsmith.shared.getValueForFeature(withID: "healthdata", forIdentity: nil) { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let value):
                    guard let jsonString = value?.stringValue, !jsonString.isEmpty else {
                        Logger.shared.log("Remote config value is nil or empty. Opening native app.")
                        viewController.openApp()
                        return
                    }
                    
                    Logger.shared.log("Received JSON string from Flagsmith: \(jsonString)")
                    
                    // ИЗМЕНЕНО: Парсим JSON, чтобы извлечь URL-шаблон из поля "about"
                    self.parseJSONString(jsonString) { urlTemplate in
                        guard !urlTemplate.isEmpty else {
                            Logger.shared.log("Parsed URL template is empty. Opening native app.")
                            viewController.openApp()
                            return
                        }
                        
                        Logger.shared.log("Parsed URL template: \(urlTemplate)")
    ...
    // НОВЫЙ КОД: Возвращаем метод для парсинга JSON
    private func parseJSONString(_ jsonString: String, completion: @escaping (String) -> Void) {
        guard let jsonData = jsonString.data(using: .utf8) else {
            Logger.shared.log("Failed to convert JSON string to Data.")
            completion("")
            return
        }
